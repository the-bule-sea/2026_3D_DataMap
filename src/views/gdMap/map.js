...
import { infoDataManager } from "./map/infoDataManager"  // 导入数据管理器
...
export class World extends Mini3d {
  constructor(canvas, assets) {
    super(canvas)
    // 中心坐标
    this.geoProjectionCenter = [121.4737, 31.2304]
    // 缩放比例
    this.geoProjectionScale = 200
    // 飞线中心
    this.flyLineCenter = [121.4737, 31.2304]
    // 地图拉伸高度
    this.depth = 0.5
    this.mapFocusLabelInfo = {
      name: "上海市",
      enName: "SHANGHAI",
      center: [121.4737, 30.2304],
    }
    // 是否点击
    this.clicked = false
    // 雾
    ...
    // 相机初始位置（针对上海市优化）
    // Z轴距离减小，让相机更靠近地图，使上海市显示更大
    this.camera.instance.position.set(0, 12, 20)  // X=0居中, Y=12高度, Z=25距离（原来39）
  ...
  init() {
    ...
    // 旋转圆环1动画（已注释，因为createRotateBorder被禁用）
    /*
    tl.to(
    ...
    */
    // 旋转圆环2动画（已注释，因为createRotateBorder被禁用）
    /*
    tl.to(
  ...
  createBar() {
    let self = this
    let data = sortByValue(provincesData).filter((item, index) => index < 3)
  ...
  createQuan(position, index) {
    ...
    // 缩小光圈大小（原来0.5），适配上海市小面积
    let geometry = new PlaneGeometry(0.3, 0.3)
  ...
  createInfoPoint() {
    ...
    this.InfoPointGroup.visible = true  // 改为true方便调试
        ...
        ` <div class="info-point-wrap">
          <div class="info-point-wrap-inner">
            ...
            <div class="info-point-content">
              ...
              <div class="content-item"><span class="label">缺陷数量</span><span class="value">${data.value}</span></div>
  ...
  /**
   * 启动监测点数据实时监测
   * @param {number} interval - 更新间隔（毫秒），默认5秒
   */
  startMonitoring(interval = 5000) {
    console.log('🔴 启动实时监测，更新间隔:', interval + 'ms')

    // 订阅数据更新
    this.dataUnsubscribe = infoDataManager.subscribe((newData) => {
      console.log('📊 接收到新的监测数据:', newData)
      this.updateInfoPoints(newData)
    })

    // 启动自动更新
    infoDataManager.startAutoUpdate(interval)
  }
   ...
   */
  stopMonitoring() {
    console.log('⏹️ 停止实时监测')

    // 停止数据管理器
    infoDataManager.stopAutoUpdate()
  ...
  /**
   * 更新监测点显示
   * @param {Array} newData - 新的监测点数据
   */
  updateInfoPoints(newData) {
    if (!this.InfoPointGroup || !newData || newData.length === 0) return

    // 清除现有的监测点
    while (this.InfoPointGroup.children.length > 0) {
      const child = this.InfoPointGroup.children[0]
      if (child.material) child.material.dispose()
      if (child.geometry) child.geometry.dispose()
      this.InfoPointGroup.remove(child)
    }
    ...
    // 重新创建监测点
    const texture = this.assets.instance.getResource("point")
    const colors = [0xfffef4, 0x77fbf5, 0xff6b6b]
    const sortedData = [...newData].sort((a, b) => b.value - a.value)
    const max = sortedData[0]?.value || 100
    ...
    sortedData.forEach((data, index) => {
      ...
      const [lx, ly] = this.geoProjection([data.lng, data.lat])
        ...
        `<div class="info-point-wrap">
          <div class="info-point-wrap-inner">
            ...
            <div class="info-point-content">
              ...
              <div class="content-item"><span class="label">缺陷数量</span><span class="value">${data.value}</span></div>
              <div class="content-item"><span class="label">等级</span><span class="value">${data.level}</span></div>
              ${data.timestamp ? `<div class="content-item"><span class="label">更新时间</span><span class="value">${data.timestamp}</span></div>` : ''}
            </div>
          </div>
        </div>`,
        new Vector3(lx, -ly, this.depth + 1.9)
      )
      ...
      sprite.addEventListener("mousedown", (ev) => {
        ...
        this.clicked = true
        this.infoPointIndex = ev.target.userData.index
        this.infoLabelElement.forEach(l => l.visible = false)
        label.visible = true
        this.createInfoPointLabelLoop()
      })
    ...
    console.log(`✅ 监测点已更新，当前${sortedData.length}个点`)
  }
